...
import static com.rishabhSoft.selenium.JCDecaux.drivers.ReadObjectProperties.*;
...
public class CommonActions extends TestNG_WebDriver {
	...
	public static String getTextFieldValue(LocatorType locator) {

		return webDriver.findElement(locator.execute(objLocatorValue)).getAttribute("value");
	}

	public static boolean isElementEnabled(LocatorType locator){
		if(webDriver.findElement(locator.execute(objLocatorValue)).isEnabled())
			return true;
		else
			return false;
	}
	
	public static boolean isElementSelected(LocatorType locator){
		if(webDriver.findElement(locator.execute(objLocatorValue)).isSelected())
			return true;
		else
			return false;
	}
	...
	public static String getSelectedLabel(LocatorType locator) {
		String selectedLabel = new Select(webDriver.findElement(locator.execute(objLocatorValue))).getFirstSelectedOption().getText();
		return selectedLabel;
	}
	
	// To Check given label is selected if not than select option by label in drop down.
	public static void checkLabelSelected(LocatorType locator, String label) {
		String selectedLabel = new Select(webDriver.findElement(locator.execute(objLocatorValue))).getFirstSelectedOption().getText();
		if(!selectedLabel.equals(label))
			new Select(webDriver.findElement(locator.execute(objLocatorValue))).selectByVisibleText(label);
	}
	...
	public static void childWindowHandler() {
	...
	public static void pageScrollDown(int x, int y){
	...
	public static void handelJavaScript(String elementID) {
		...
		((JavascriptExecutor) webDriver).executeScript("document.getElementById('elementID').click();");
	}
